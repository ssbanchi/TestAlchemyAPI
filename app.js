...
function example(req, res) {
  ...
  //image(req, res, output);//image API
  //image_img(req, res, output);//image API
}
...
function image_img(req, res, output) {
	//alchemyapi.image_keywords('image', 'emaxfpo.jpg', {}, function(response) {       
  alchemyapi.image_keywords('image', 'aaa.jpg', {}, function(response) {
		output['image'] = { url:demo_url, response:JSON.stringify(response,null,4), results:response };
		image_keywords(req, res, output);
	});
}

//access.alchemyapi.com/calls/url/URLGetRankedImageKeywords
